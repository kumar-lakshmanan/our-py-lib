...
class Table():
    ...
    def populateTable(self, widget, listOfList=[], append=0, rowHeight=15, insertAtFirst=False, autoResize=True, progressDialog=None):
        self.widget = widget
        if not append: self.clearTable(self.widget)
        sortingVal = self.widget.isSortingEnabled()
        self.widget.blockSignals(1)
        ...
        if progressDialog: progressDialog.progress.setMaximum(ttlRows)
        for row in xrange(0,ttlRows):
            ...
            self.__progress(progressDialog, row, ttlRows)
    ...
    def __progress(self, progressBar, currentVal, totalVal):
        if not progressBar: return
        progressBar.label.setText('Loading... %s of %s' % (currentVal, totalVal))
        progressBar.progress.setValue(currentVal)
    ...
    def getItemsToLabel(self, items, getFormat='Label'):
        res = []
        if type(items)==type([]):
            for eachR in items:
                if type(eachR)==type([]):
                    r1 = []
                    for eachC in eachR:
                        txt = self.getItemLabel(eachC)[getFormat]
                        r1.append(txt)
                    res.append(r1)
                else:
                    txt = self.getItemLabel(eachR)[getFormat]
                    res.append(txt)
        return res
