...
#UI Packs
from aov_dlg_ui import Ui_Dialog
...
class AOVDialog(QtGui.QDialog, Ui_Dialog):
    ...
    def sigListItemChange(self, *arg):
        itm = self.qtsList.getSelectedItemLabels(self.listWidget)
        if len(itm)>1:
##            for it in itm:
##                self.matter.setEnabled(0)
##                if it == 'matte':
##                    self.matter.setEnabled(1)
##                    break;
            self.le3.setText('')
            self.le1.setText('Multi Selected')
            self.le1.setEnabled(0)
        elif len(itm)>0:
            self.le1.setEnabled(1)
            self.le1.setText(itm[0])
        else:
            self.le1.setText('')
            self.le3.setText('')
