...
#UI Packs
from passScene_dlg_ui import Ui_Dialog
...
class PassSceneDialog(QtGui.QDialog, Ui_Dialog):
    ...
    def sigConnection(self):
        ...
        #Custom
        self.connect(self.listWidget, QtCore.SIGNAL('itemSelectionChanged()'), self.__applySelectedLabel )
    ...
    def __applySelectedLabel(self, *arg):
        lbl = self.qtsList.getSelectedItemLabel(self.listWidget)
        self.lineEdit_4.setText(lbl)
...
if __name__ == '__main__':
    ...
    cfg.btnOKLabel = 'Add'
    cd = PassSceneDialog(app, cfg)
    cd.initialize()
    cd.show()
    ec = app.exec_()
    print cd.lineEdit_2.text()
    sys.exit(ec)